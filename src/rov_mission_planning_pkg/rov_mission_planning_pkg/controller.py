...
class MoveToStart(State):
    def __init__(self, controller, limit_l, limit_r, start_depth, limit_depth, distance, publisher,\
        ...
        self.pid_x = PID(1.0, 0.0, 0.0, setpoint=limit_l)
        self.pid_x.sample_time = 0.01  # Update every 0.01 seconds
        self.pid_x.output_limits = (-1.0, 1.0) 

        self.pid_y = PID(1.0, 0.0, 0.0, setpoint=start_depth)
        self.pid_y.sample_time = 0.01  # Update every 0.01 seconds
        self.pid_y.output_limits = (-1.0, 1.0)

        self.pid_z = PID(1.0, 0.0, 0.0, setpoint=distance)
        self.pid_z.sample_time = 0.01  # Update every 0.01 seconds
    ...
    def position_callback(self, msg: Vector3Stamped):
        ...
        if self.start_check >= 10:
            new_state = LeftToRight(self.controller, self.limit_l, self.limit_r, self.depths, 0, \
                self.distance, self.publisher, MOVE_TO_START, self.x_discretization, self.tolerance)
...
class LeftToRight(State):
    ...
    def __init__(self, controller, limit_l, limit_r, depths, depth_level, distance, publisher, prev_state,\
        ...
        self.pid_y = PID(1.0, 0.0, 0.0, setpoint=depths[depth_level])
        self.pid_y.sample_time = 0.01  # Update every 0.01 seconds
        self.pid_y.output_limits = (-1.0, 1.0)
    ...
    def position_callback(self, msg):
        ...
        if pub_msg.linear.x == 0.0 and pub_msg.linear.y == 0.0 and pub_msg.linear.z == 0.0:
            ...
            if self.photos_taken_at_pos == 5:
                if self.dscrtz_idx == len(self.X) - 1:
                    new_state = Dive(self.controller, self.limit_l, self.limit_r, self.depths,\
                        self.depth_level + 1, self.distance, self.publisher, LEFT_TO_RIGHT, self.x_discretization, self.tolerance)
                    self.controller.change_state(new_state)
                else:
                    self.dscrtz_idx += 1
                    self.photos_taken_at_pos = 0
...
class RightToLeft(State):
    ...
    def __init__(self, controller, limit_l, limit_r, depths, depth_level, distance, publisher, prev_state,\
        ...
        self.pid_x = PID(1.0, 0.0, 0.0, setpoint=limit_r)
        self.pid_x.sample_time = 0.01  # Update every 0.01 seconds
        self.pid_x.output_limits = (-1.0, 1.0) 
    ...
    def position_callback(self, msg):
        ...
        if pub_msg.linear.x == 0.0 and pub_msg.linear.y == 0.0 and pub_msg.linear.z == 0.0:
            ...
            if self.photos_taken_at_pos == 5:
                if self.dscrtz_idx == len(self.X) - 1:
                    new_state = Dive(self.controller, self.limit_l, self.limit_r, self.depths,\
                        self.depth_level + 1, self.distance, self.publisher, RIGHT_TO_LEFT, self.x_discretization, self.tolerance)
                    self.controller.change_state(new_state)
                else:
                    self.dscrtz_idx += 1
                    self.photos_taken_at_pos = 0
...
class Dive(State):
    def __init__(self, controller, limit_l, limit_r, depths, depth_level, distance, publisher, prev_state,\
        ...
        if depth_level >= len(depths):
            new_state = Resurface(controller, 0.0, 0.0, 0.0)
            controller.change_state(new_state)
            return
        ...
        if prev_state == LEFT_TO_RIGHT:
            ...
            x_setpoint = limit_l
        self.pid_x = PID(1.0, 0.0, 0.0, setpoint=x_setpoint)
        self.pid_x.sample_time = 0.01  # Update every 0.01 seconds
        self.pid_x.output_limits = (-1.0, 1.0) 
...
class Resurface(State):
    def __init__(self, controller, x, y, z):
        ...
        self.pid_x = PID(1.0, 0.0, 0.0, setpoint=x)
        self.pid_x.sample_time = 0.01  # Update every 0.01 seconds
        self.pid_x.output_limits = (-1.0, 1.0) 

        self.pid_y = PID(1.0, 0.0, 0.0, setpoint=y)
        self.pid_y.sample_time = 0.01  # Update every 0.01 seconds
        self.pid_y.output_limits = (-1.0, 1.0)

        self.pid_z = PID(1.0, 0.0, 0.0, setpoint=z)
        self.pid_z.sample_time = 0.01  # Update every 0.01 seconds
        self.pid_z.output_limits = (-1.0, 1.0)
...
class Controller(Node):
        ...
        def initialize_publishers(self):
            ...
            self.slow_gain_pub = self.create_publisher(Float32, self.get_parameter(SLOW_GAIN_TOPIC_PARAM), 10)
        ...
        def initialize_class_objects(self):
            ...
            self.current_state = MoveToStart(self, limit_l, limit_r, start_vector[1], limit_depth,\
                start_vector[2], self.thruster_pub, x_discretization, depth_discretization, tolerance)
        ...
        def set_slow_gain(self, value):
            self.slow_gain_pub.publish(value)
